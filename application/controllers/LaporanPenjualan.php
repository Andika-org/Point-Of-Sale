...
public	function index() 
	{
		...
		$data["tglawal"] = "";
		$data["grandtotal"] =  $this->Model_Laporanpenjualan->sumtotal($tglawal,$tglakhir);
		$data["tglakhir"] = "";
		$data["detailpenjualan"] = "";
		$data["toko"] = $this->Model_Laporanpenjualan->datatoko();
		$this->load->view('admin/Menuadmin');
		$this->load->view('admin/laporanpenjualan/ViewLaporanpenjualan',$data);
	}
public	function lihatreportpenjualan() 
	{	
		$data["tglawal"] = $this->input->post("Tanggal_Awal");
		$data["tglakhir"] = $tglakhir = $this->input->post("Tanggal_Akhir");
		
		$tglawal = date("Y-m-d",strtotime($data["tglawal"]));
		$tglakhir = date("Y-m-d",strtotime($data["tglakhir"]));
		
		$print ="";
		
		$data["datapenjualan"] = $this->Model_Laporanpenjualan->datatbpenjualan($tglawal,$tglakhir);
		
		$i = 1;
		foreach($data["datapenjualan"] as $dtpenjualan){
			
			$print = $print .'
					<tr style="border:1px solid #000;">
					<td align="center" style="border:1px solid #000;">'.$i++.'</td>
					<td style="padding-left:10px;border:1px solid #000;">'.$dtpenjualan->Nota.'</td>
					<td align="center" style="border:1px solid #000;">'.date('d F Y',strtotime($dtpenjualan->Tanggal_Penjualan)).'</td>
					<td align="center" style="border:1px solid #000;">'.$dtpenjualan->Discount.' %'.'</td>
					<td align="center" style="border:1px solid #000;">'.$dtpenjualan->Ppn.' %'.'</td>
					<td align="left" style="border:1px solid #000;padding-left:10px;">'.'Rp. '.number_format($dtpenjualan->Total_Pembelian,0,",",".").'</td>';
					
					$notajual = $dtpenjualan->Nota;
					$data["totalitem"] = $this->Model_Laporanpenjualan->hitungitem($notajual);
					foreach($data["totalitem"] as $titem){
									$print = $print .'
					<td align="center" style="border:1px solid #000;">'.$titem->TotalItem.' Item'.'</td>';
					}
					
					$nota = $dtpenjualan->Nota;
					
					$data["datadetail"] = $this->Model_Laporanpenjualan->listtbpenjualandetail($nota);
					if(!empty($data["datadetail"])){
						
						$print = $print.'
						<td align="center" style="border:1px solid #000;"><table>';
						
						foreach($data["datadetail"] as $dd){
							$print = $print .'
								<tr><td style="padding-left:10px;">'.$dd->Kode_Bahan_Baku.'</td></tr>
								';
						}
						...
						foreach($data["datadetail"] as $dd){
							$print = $print .'
								<tr><td style="padding-left:10px;">'.$dd->Nama_Barang.'</td></tr>
								';
						}
						...
						foreach($data["datadetail"] as $dd){
							$print = $print .'
								<tr><td style="padding-left:10px;">'.$dd->Qty ." ".$dd->Nama_Satuan.'</td></tr>
								';
						}
						...
						foreach($data["datadetail"] as $dd){
							$print = $print .'
								<tr><td align="" style="padding-left:10px;">'.'Rp. '.number_format($dd->Harga_Jual,0,",",".").'</td></tr>
								';
						}
		...
		$data["grandtotal"] = $this->Model_Laporanpenjualan->sumtotal($tglawal,$tglakhir);
		$data["detailpenjualan"] = $print;
		$data["toko"] = $this->Model_Laporanpenjualan->datatoko();
		$this->load->view('admin/Menuadmin');
		$this->load->view('admin/laporanpenjualan/ViewLaporanpenjualan',$data);
	}
public	function cetakexcelreportpenjualan() 
	{	
		$data["tglawal"] = $this->uri->segment(3);
		$data["tglakhir"] = $this->uri->segment(4);
		...
		foreach($data["datapenjualan"] as $dtpenjualan){
			...
			$print = $print .'
					...
					if(!empty($data["datadetail"])){
						...
						foreach($data["datadetail"] as $dd){
							$print = $print .'
								<tr><td style="padding-left:10px;border-right:1px solid black;">'.$dd->Kode_Bahan_Baku.'</td></tr>
								';
						}
						...
						foreach($data["datadetail"] as $dd){
							$print = $print .'
								<tr><td style="padding-left:10px;border-right:1px solid black;">'.$dd->Nama_Barang.'</td></tr>
								';
						}
						...
						foreach($data["datadetail"] as $dd){
							$print = $print .'
								<tr><td style="padding-left:10px;border-right:1px solid black;">'.$dd->Qty ." ".$dd->Nama_Satuan.'</td></tr>
								';
						}
						...
						foreach($data["datadetail"] as $dd){
							$print = $print .'
								<tr><td align="" style="padding-left:10px;border-right:1px solid black;">'.'Rp. '.number_format($dd->Harga_Jual,0,",",".").'</td></tr>
								';
						}
		...
		$this->load->view('admin/laporanpenjualan/CetakExcelLaporanpenjualan',$data);
	}
/////////// batas baru ////////////
public	function indexx($offset = 0) 
	{
